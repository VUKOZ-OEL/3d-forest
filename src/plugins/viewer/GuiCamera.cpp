...
void GuiCamera::setViewport(int x, int y, int width, int height)
{
    if (width < 1)
    {
        width = 1;
...
void GuiCamera::updateProjection()
{
    if (perspective_)
    {
        setPerspective();
...
void GuiCamera::setOrthographic()
{
    perspective_ = false;
...
void GuiCamera::setPerspective(float fovy, float zNear, float zFar)
{
    float w = static_cast<float>(viewport_.width());
    float h = static_cast<float>(viewport_.height());
    float aspect = w / h;
...
void GuiCamera::setPerspective(float fovy,
                               float aspect,
                               float zNear,
                               float zFar)
{
...
void GuiCamera::setLookAt(const QVector3D &center, float distance)
{
    center_ = center;
...
void GuiCamera::setLookAt(const QVector3D &eye,
...
void GuiCamera::setDistance(float distance)
{
    if (distance < 1.0F)
    {
        distance = 1.0F;
...
EditorCamera GuiCamera::toCamera() const
{
    EditorCamera ret;
...
QVector3D GuiCamera::project(const QVector3D &world) const
{
...
QVector3D GuiCamera::unproject(const QVector3D &window) const
{
    QVector3D p = window;
    p.setY(static_cast<float>(viewport_.bottom()) - p.y());
...
void GuiCamera::getRay(int x, int y, QVector3D *base, QVector3D *direction)
{
    QVector3D pt;
...
void GuiCamera::mousePressEvent(QMouseEvent *event)
{
    mouseLastPosition_ = event->pos();
}

void GuiCamera::mouseMoveEvent(QMouseEvent *event)
{
...
void GuiCamera::wheelEvent(QWheelEvent *event)
{
...
void GuiCamera::rotate(int dx, int dy)
{
...
void GuiCamera::pan(int dx, int dy)
{
    float x = static_cast<float>(dx) / static_cast<float>(viewport_.width());
    float y = static_cast<float>(-dy) / static_cast<float>(viewport_.height());
...
void GuiCamera::setModelView(const QMatrix4x4 &m)
{
    modelView_ = m;
...
void GuiCamera::setProjection(const QMatrix4x4 &m)
{
    projection_ = m;
...
void GuiCamera::setModelViewProjection(const QMatrix4x4 &m)
{
