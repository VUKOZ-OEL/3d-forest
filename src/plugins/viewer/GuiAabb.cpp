...
void GuiAabb::set(const QVector3D &min, const QVector3D &max)
{
    min_ = min;
    max_ = max;
...
void GuiAabb::set(const std::vector<float> &xyz)
{
...
void GuiAabb::set(const Box<float> &box)
{
    set(box.min(0), box.min(1), box.min(2), box.max(0), box.max(1), box.max(2));
}

void GuiAabb::set(const Box<double> &box)
{
...
void GuiAabb::extend(const GuiAabb &box)
{
...
void GuiAabb::invalidate()
{
    valid_ = false;
}

void GuiAabb::setValid()
{
...
void GuiAabb::getRange(const QVector3D &eye,
