...
/** @file ElevationFilterWidget.cpp */
...
// Include local.
#define LOG_MODULE_NAME "ElevationFilterWidget"
#define LOG_MODULE_DEBUG_ENABLED 1
#include <Log.hpp>

ElevationFilterWidget::ElevationFilterWidget(MainWindow *mainWindow)
    : QWidget(mainWindow),
...
void ElevationFilterWidget::slotUpdate(void *sender,
...
void ElevationFilterWidget::setElevation(const Range<double> &range)
{
    LOG_DEBUG(<< "Set elevation filter range <" << range << ">.");

    elevationRange_ = range;
    double min = elevationRange_.minimum() * 100.0;
    double max = elevationRange_.maximum() * 100.0;
...
void ElevationFilterWidget::filterChanged()
{
...
void ElevationFilterWidget::setFilterEnabled(bool b)
{
...
void ElevationFilterWidget::slotRangeIntermediateMinimumValue()
{
    LOG_DEBUG(<< "Minimum value changed.");

    elevationRange_.setMinimumValue(elevationInput_->minimumValue() * 0.01);
    filterChanged();
}

void ElevationFilterWidget::slotRangeIntermediateMaximumValue()
{
    LOG_DEBUG(<< "Maximum value changed.");

    elevationRange_.setMaximumValue(elevationInput_->maximumValue() * 0.01);
    filterChanged();
}
