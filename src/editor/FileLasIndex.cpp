...
const uint32_t FileLasIndex::CHUNK_TYPE = 0x38584449U; /**< Signature "IDX8" */
...
void FileLasIndex::translate(const Vector3<double> &v)
{
...
void FileLasIndex::selectLeaves(std::vector<SelectionTile> &selection,
...
void FileLasIndex::selectLeaves(std::vector<Selection> &selection,
                                const Box<double> &window,
                                size_t id) const
{
    if (!empty())
...
void FileLasIndex::selectNodes(std::vector<Selection> &selection,
                               const Box<double> &window,
                               size_t id) const
{
    if (!empty())
...
const FileLasIndex::Node *FileLasIndex::selectNode(
...
const FileLasIndex::Node *FileLasIndex::selectLeaf(double x,
                                                   double y,
                                                   double z) const
{
    if (size() > 0)
...
void FileLasIndex::divide(Box<double> &boundary,
                          double x,
                          double y,
                          double z,
                          uint64_t code) const
...
const FileLasIndex::Node *FileLasIndex::root() const
{
    return &nodes_[0];
}

const FileLasIndex::Node *FileLasIndex::next(const Node *node, size_t idx) const
{
    if (node->next[idx])
    {
...
const FileLasIndex::Node *FileLasIndex::prev(const Node *node) const
{
    if (node == root())
    {
        return nullptr;
...
Box<double> FileLasIndex::boundary(const Node *node,
...
void FileLasIndex::insertBegin(const Box<double> &boundary,
...
uint64_t FileLasIndex::insertEndToLeaves(Node *data,
                                         BuildNode *node,
                                         uint32_t prev,
                                         uint32_t &idx,
                                         uint64_t &from)
...
size_t FileLasIndex::countNodes() const
{
    if (root_)
    {
...
size_t FileLasIndex::countNodes(BuildNode *node) const
{
    size_t n = 1;
...
uint64_t FileLasIndex::insert(double x, double y, double z)
{
    uint64_t code = 0;
    uint64_t ecode = 0;
    uint64_t c;
...
void FileLasIndex::read(const std::string &path)
{
    FileChunk file;
    file.open(path, "r");
    read(file);
...
void FileLasIndex::read(const std::string &path, uint64_t offset)
{
    FileChunk file;
    file.open(path, "r");
    file.seek(offset);
...
void FileLasIndex::read(FileChunk &file)
{
    // Chunk header
...
void FileLasIndex::readPayload(FileChunk &file, const FileChunk::Chunk &chunk)
{
    file.validate(chunk,
                  CHUNK_TYPE,
...
void FileLasIndex::write(const std::string &path) const
{
    FileChunk file;
    file.open(path, "w");
    write(file);
...
void FileLasIndex::write(FileChunk &file) const
{
    // Chunk
    FileChunk::Chunk chunk;
...
Json &FileLasIndex::write(Json &out) const
{
    if (size() > 0)
    {
...
Json &FileLasIndex::write(Json &out, const Node *data, size_t idx) const
{
...
std::ostream &operator<<(std::ostream &os, const FileLasIndex &obj)
{
    Json json;
