...
EditorPage::EditorPage(Editor *editor,
                       EditorQuery *query,
                       uint32_t datasetId,
                       uint32_t pageId)
    : selectionSize(0),
...
void EditorPage::read()
{
    ...
    const FileLasIndex::Node *node = dataset.index().at(pageId_);
    ...
    pathIndex = FileLasIndexBuilder::extension(dataset.path());
...
void EditorPage::selectBox()
{
    ...
    {
        const FileLasIndex::Node *nodeL2 = octree.at(selectedNodes_[i].idx);
        if (!nodeL2)
        {
            continue;
        }
