...
EditorQuery::EditorQuery(Editor *editor) : editor_(editor)
{
    maximumResults_ = 0;
    cacheSizeMax_ = 200;
}
...
void EditorQuery::exec(
    const std::vector<FileLasIndex::Selection> &selectedPages)
{
...
bool EditorQuery::nextPage()
{
    ...
    while (pageIndex_ < selectedPages_.size())
    {
        LOG_EDITOR_QUERY("pageIndex " << pageIndex_ << "/"
                                      << selectedPages_.size());
        FileLasIndex::Selection &selectedPage = selectedPages_[pageIndex_];
...
void EditorQuery::selectCamera(const EditorCamera &camera)
{
    ...
    while (!queue.empty() && lru_.size() < cacheSizeMax_)
    {
        ...
        const FileLasIndex &index = db.index();
        ...
        for (size_t i = 0; i < 8; i++)
        {
            if (node->next[i])
            {
                const FileLasIndex::Node *sub = index.at(node->next[i]);
