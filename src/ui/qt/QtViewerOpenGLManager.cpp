...
/** @file QtViewerOpenGLManager.cpp */

// Include 3D Forest.
#include <QtViewerOpenGLManager.hpp>
#include <QtViewerOpenGLStrokeFont.hpp>
#include <Util.hpp>
...
// Include local.
#define LOG_MODULE_NAME "QtViewerOpenGLManager"
// #define LOG_MODULE_DEBUG_ENABLED 1
#include <Log.hpp>

size_t QtViewerOpenGLManager::nextManagerId_ = 0;

QtViewerOpenGLManager::QtViewerOpenGLManager()
    : initialized_(false),
      lastAccess_(nullptr),
      lastAccessIdx_(-1),
      nDisplayLists_(0),
...
// Initialization.
void QtViewerOpenGLManager::init()
{
...
// Release.
void QtViewerOpenGLManager::release(bool keep_)
{
...
// Create new resource.
GLuint QtViewerOpenGLManager::createResource(
    QtViewerOpenGLResource *owner,
    size_t id,
    ...
    GLsizei range)
{
    ...
    if (ownerIdx != SIZE_MAX)
    {
        ...
        // Create.
        if (res == QtViewerOpenGLManager::DisplayList)
        {
        ...
        else if (res == QtViewerOpenGLManager::Texture)
        {
            ASSERT(range == 1);
        ...
        else if (res == QtViewerOpenGLManager::BufferARB)
        {
            ASSERT(range == 1);
            if (glGenBuffersARB)
            {
        ...
        else if (res == QtViewerOpenGLManager::ProgramARB)
        {
            ASSERT(range == 1);
            if (glGenProgramsARB)
            {
...
// Get resource.
GLuint QtViewerOpenGLManager::resource(QtViewerOpenGLResource *owner, size_t id)
{
...
// Release resource.
void QtViewerOpenGLManager::releaseResource(QtViewerOpenGLResource *owner,
                                            size_t id)
{
...
void QtViewerOpenGLManager::releaseResources(QtViewerOpenGLResource *owner)
{
...
void QtViewerOpenGLManager::removeOwner(QtViewerOpenGLResource *owner)
{
    size_t idxRes;
    size_t idx;
    size_t i;
...
void QtViewerOpenGLManager::updateResources()
{
    size_t i = 0;
    ...
    if (n > 0)
    {
        for (i = 0; i < n; ++i)
        {
            ...
            {
                if (toDelete_[i].resource == QtViewerOpenGLManager::DisplayList)
                {
                    SAFE_GL(
                         ...
                         QtViewerOpenGLManager::Texture)
                {
                    ...
                    LOG_DEBUG(<< "Update resources nTextures <" << nTextures_
                         ...
                         QtViewerOpenGLManager::BufferARB)
                {
                    ...
                    LOG_DEBUG(<< "Update resources nBuffers <" << nBuffers_
                         ...
                         QtViewerOpenGLManager::ProgramARB)
                {
...
GLuint QtViewerOpenGLManager::font()
{
    GLuint displayList;
...
std::string QtViewerOpenGLManager::cleanText(const std::string &text)
{
...
float QtViewerOpenGLManager::textWidth(const std::string &text)
{
    ASSERT(strokeFont_);
...
float QtViewerOpenGLManager::textHeight()
{
    ASSERT(strokeFont_);
...
void QtViewerOpenGLManager::updateGL()
{
    strokeFont_ = std::make_shared<QtViewerOpenGLStrokeFont>();
}
