...
/** @file QtViewerOpenGLManager.hpp */

#ifndef QT_VIEWER_OPEN_GL_MANAGER_HPP
#define QT_VIEWER_OPEN_GL_MANAGER_HPP
...
class QtViewerOpenGLResource;
class QtViewerOpenGLStrokeFont;

/** Qt Viewer OpenGL Manager. */
...
public:
    ...
    QtViewerOpenGLManager();
    virtual ~QtViewerOpenGLManager();
    ...
    GLuint createResource(QtViewerOpenGLResource *owner,
                          size_t id,
                          QtViewerOpenGLManager::ResourceType res,
                          GLsizei range = 1);
    ...
    GLuint resource(QtViewerOpenGLResource *owner, size_t id);

    /** Release resource.
        \param owner Specifies the owner by a pointer.
        \param id Specifies the resource to release. */
    void releaseResource(QtViewerOpenGLResource *owner, size_t id);

    /** Release all owners resources and remove the entry.
        \param owner Specifies the owner by a pointer. */
    void releaseResources(QtViewerOpenGLResource *owner);

    /** Remove owner from the manager.
        \param owner Specifies the owner by a pointer. */
    void removeOwner(QtViewerOpenGLResource *owner);
...
protected:
    ...
    std::vector<QtViewerOpenGLResource *> owners_;
    ...
    std::shared_ptr<QtViewerOpenGLStrokeFont> strokeFont_;

public:
    // texture
    QtViewerOpenGL::PFNGLACTIVETEXTUREARBPROC glActiveTextureARB;

    // misc
    QtViewerOpenGL::PFNGLDRAWRANGEELEMENTSPROC glDrawRangeElements;

    // shader
    QtViewerOpenGL::PFNGLGENPROGRAMSARBPROC glGenProgramsARB;
    QtViewerOpenGL::PFNGLDELETEPROGRAMSARBPROC glDeleteProgramsARB;
    QtViewerOpenGL::PFNGLBINDPROGRAMARBPROC glBindProgramARB;
    QtViewerOpenGL::PFNGLPROGRAMSTRINGARBPROC glProgramStringARB;
    QtViewerOpenGL::PFNGLPROGRAMENVPARAMETER4FARBPROC
        glProgramEnvParameter4fARB;
    QtViewerOpenGL::PFNGLPROGRAMENVPARAMETER4FVARBPROC
        glProgramEnvParameter4fvARB;

    // VBO
    QtViewerOpenGL::PFNGLBINDBUFFERARBPROC glBindBufferARB;
    QtViewerOpenGL::PFNGLDELETEBUFFERSARBPROC glDeleteBuffersARB;
    QtViewerOpenGL::PFNGLGENBUFFERSARBPROC glGenBuffersARB;
    QtViewerOpenGL::PFNGLISBUFFERARBPROC glIsBufferARB;
    QtViewerOpenGL::PFNGLBUFFERDATAARBPROC glBufferDataARB;
};

#endif /* QT_VIEWER_OPEN_GL_MANAGER_HPP */