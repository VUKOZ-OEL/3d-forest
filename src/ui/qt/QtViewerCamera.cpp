...
// Include local.
#define LOG_MODULE_NAME "QtViewerCamera"
// #define LOG_MODULE_DEBUG_ENABLED 1
#include <Log.hpp>

QtViewerCamera::QtViewerCamera()
    : eye_(0.0F, 0.0F, 1.0F),
...
void QtViewerCamera::setViewportId(size_t id)
{
    viewportId_ = id;
}

void QtViewerCamera::setViewport(int x, int y, int width, int height)
{
    if (width < 1)
    {
        width = 1;
...
void QtViewerCamera::updateProjection()
{
    if (perspective_)
    {
        setPerspective();
...
void QtViewerCamera::setPerspective()
{
    perspective_ = true;
...
void QtViewerCamera::setOrthographic()
{
    perspective_ = false;
...
void QtViewerCamera::setPerspective(float fovy, float zNear, float zFar)
{
    float w = static_cast<float>(viewport_.width());
    float h = static_cast<float>(viewport_.height());
    float aspect = w / h;
...
void QtViewerCamera::setPerspective(float fovy,
                                    float aspect,
                                    float zNear,
                                    float zFar)
{
...
void QtViewerCamera::setLookAt(const QVector3D &center, float distance)
{
    center_ = center;
...
void QtViewerCamera::setLookAt(const QVector3D &eye,
...
void QtViewerCamera::setLookAt(const QVector3D &dir,
                               float distance,
                               const QVector3D &center,
                               const QVector3D &up)
{
...
void QtViewerCamera::setDistance(float distance)
{
    if (distance < 1.0F)
    {
        distance = 1.0F;
...
Camera QtViewerCamera::toCamera() const
{
    Camera ret;
...
QVector3D QtViewerCamera::project(const QVector3D &world) const
{
...
QVector3D QtViewerCamera::unproject(const QVector3D &window) const
{
    QVector3D p = window;
    p.setY(static_cast<float>(viewport_.bottom()) - p.y());
...
void QtViewerCamera::ray(int x, int y, QVector3D *near, QVector3D *far)
{
    QVector3D pt;
...
void QtViewerCamera::mousePressEvent(QMouseEvent *event)
{
    mouseLastPosition_ = event->pos();
}

void QtViewerCamera::mouseMoveEvent(QMouseEvent *event)
{
...
void QtViewerCamera::wheelEvent(QWheelEvent *event)
{
...
void QtViewerCamera::rotate(int dx, int dy)
{
...
void QtViewerCamera::pan(int dx, int dy)
{
    float x = static_cast<float>(dx) / static_cast<float>(viewport_.width());
    float y = static_cast<float>(-dy) / static_cast<float>(viewport_.height());
...
void QtViewerCamera::updateMatrix()
{
    QMatrix4x4 m;
...
void QtViewerCamera::setModelView(const QMatrix4x4 &m)
{
    modelView_ = m;
...
void QtViewerCamera::setProjection(const QMatrix4x4 &m)
{
    projection_ = m;
...
void QtViewerCamera::setModelViewProjection(const QMatrix4x4 &m)
{
...
void QtViewerCamera::updateFrustrum()
{
