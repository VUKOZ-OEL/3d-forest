...
/** @file QtViewerAabb.cpp */

// Include 3D Forest.
#include <QtViewerAabb.hpp>

// Include local.
#define LOG_MODULE_NAME "QtViewerAabb"
#include <Log.hpp>

QtViewerAabb::QtViewerAabb() : valid_(false)
{
}

QtViewerAabb::~QtViewerAabb()
{
}

void QtViewerAabb::set(float min_x,
                       float min_y,
                       float min_z,
                       float max_x,
                       float max_y,
...
void QtViewerAabb::set(const QVector3D &min, const QVector3D &max)
{
    min_ = min;
    max_ = max;
...
void QtViewerAabb::set(const std::vector<float> &xyz)
{
...
void QtViewerAabb::set(const Box<float> &box)
{
    set(box.min(0), box.min(1), box.min(2), box.max(0), box.max(1), box.max(2));
}

void QtViewerAabb::set(const Box<double> &box)
{
...
void QtViewerAabb::extend(const QtViewerAabb &box)
{
...
void QtViewerAabb::invalidate()
{
    valid_ = false;
}

void QtViewerAabb::setValid()
{
...
void QtViewerAabb::range(const QVector3D &eye,
