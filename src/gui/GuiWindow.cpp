...
GuiWindow::GuiWindow(QMainWindow *parent)
    : QDockWidget(parent),
      mainWindow_(parent)
{
    // Title bar
...
QMainWindow *GuiWindow::mainWindow() const
{
    return mainWindow_;
}

void GuiWindow::setWindowTitle(const QString &text)
{
    windowTitle_->setText(text);
    QDockWidget::setWindowTitle(text);
}

void GuiWindow::setWindowIcon(const QIcon &icon)
{
...
void GuiWindow::windowDockFloatEvent(bool topLevel)
{
    if (topLevel)
    {
