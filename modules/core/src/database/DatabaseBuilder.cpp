...
DatabaseBuilder::DatabaseBuilder()
    : state_(STATE_NONE), valueTotal_(0), maximumTotal_(0)
{
}
...
std::string DatabaseBuilder::extensionL1(const std::string &path)
{
    return File::replaceExtension(path, ".idx1");
}

std::string DatabaseBuilder::extensionL2(const std::string &path)
{
    return File::replaceExtension(path, ".idx2");
}

void DatabaseBuilder::index(const std::string &outputPath,
...
double DatabaseBuilder::percent() const
{
    if (maximumTotal_ == 0)
    {
        return 100.;
...
void DatabaseBuilder::start(const std::string &outputPath,
                            const std::string &inputPath,
                            const DatabaseBuilder::Settings &settings)
{
    // Initialize
...
void DatabaseBuilder::openFiles()
{
    // Input
...
void DatabaseBuilder::nextState()
{
    value_ = 0;
    valueIdx_ = 0;
    maximum_ = 0;
...
void DatabaseBuilder::stateCopy()
{
    // Step
    uint64_t step;
    uint64_t remain;
...
void DatabaseBuilder::stateCopyPoints()
{
    // Step
    uint64_t step;
    uint64_t remainIdx;
...
void DatabaseBuilder::stateRandomize()
{
    // Step
    uint64_t step;
    uint64_t remainIdx;
...
void DatabaseBuilder::stateMove()
{
    // Move
    inputLas_.close();
    outputLas_.close();
...
void DatabaseBuilder::stateMainBegin()
{
    // Insert begin
#if 1
    // Cube
...
void DatabaseBuilder::stateMainInsert()
{
    // Step
    uint64_t step;
    uint64_t remainIdx;
...
void DatabaseBuilder::stateMainEnd()
{
...
void DatabaseBuilder::stateMainSort()
{
    // Step
    uint64_t step;
    uint64_t remainIdx;
...
void DatabaseBuilder::stateNodeBegin()
{
...
void DatabaseBuilder::stateNodeInsert()
{
    // Step
    uint64_t step;
    const OctreeIndex::Node *node;
...
void DatabaseBuilder::stateNodeEnd()
{
    indexNodeFile_.close();
}

void DatabaseBuilder::stateEnd()
{
