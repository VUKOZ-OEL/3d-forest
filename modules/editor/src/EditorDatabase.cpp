...
void EditorDatabase::setProperties(const EditorDataSet &properties)
{
    if (properties_.path() != properties.path())
    {
        readIndex(properties.path());
    }
...
{
    const std::string pathIndex = FileIndexBuilder::extension(pathLas);
